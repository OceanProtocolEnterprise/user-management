...
Includes: enrollment invitation flow, recovery flow, custom authentication flow, custom policies, mappings, stages, provider, and application
Saves generated client credentials, JWKS URI, Issuer, and Audience to .env file
Supports SMTP config from .env and web certificate discovery
"""
...
def get_participant_base_url():
    """Construct the participant base URL from environment variables."""
    hostname = os.getenv("PARTICIPANT_IDP_HOSTNAME", "")
    port = os.getenv("PARTICIPANT_IDP_PORT_HTTPS", "")
    ...
    if hostname:
        ...
        base_url = os.getenv("AUTHENTIK_BASE_URL", "https://localhost/")
...
def get_certificate_name(certs_dir: Optional[str] = None) -> Optional[str]:
    ...
    try:
        possible_paths = [
            ...
            os.path.join(os.getcwd(), "docker-compose", "authentik", "certs"),
        ]
...
def save_onboarding_config(app_slug: str, client_id: str, client_secret: str, base_url: str, participant_redirect_uris: List[str] = None):
    """Save or update the onboarding configuration to a JSON file."""
    config_file = f"config-for-onboarding-{app_slug}.json"
    ...
    if os.path.exists(config_file):
        try:
            ...
            print(f"Existing config file is invalid, will recreate: {config_file}")
            config_data = {}
    
    config_data["participant_idp_well_known_url"] = well_known_url
    config_data["participant_idp_consumer_key"] = client_id
    config_data["participant_idp_consumer_secret"] = client_secret
    config_data["authentik_app_slug"] = app_slug
    config_data["participant_redirect_uris"] = participant_redirect_uris if participant_redirect_uris else []
    config_data["central_idp_provider_name"] = os.getenv("CENTRAL_IDP_PROVIDER_NAME", "")
    
    with open(config_file, 'w') as f:
        json.dump(config_data, f, indent=2)
    
    print(f"Onboarding config saved to: {config_file}")
    return config_file
...
def generate_blueprint(
    ...
    """
    Generate the complete blueprint structure for the IDP authentik instance.
    
    Creates all necessary flows, stages, policies, mappings, provider, application,
    and certificate configuration for the IDP instance.
    """
    ...
    base_url = get_participant_base_url()
    ...
    # ================================================================
    # 7. FLOW STAGE BINDINGS FOR oe-enrollment-invitation FLOW
    # ================================================================
    ...
    # ================================================================
    # 7.5 FLOW STAGE BINDINGS FOR oe-recovery FLOW
    # ================================================================
    ...
    # ================================================================
    # 8. POLICY BINDINGS FOR oe-enrollment-invitation
    # ================================================================
    ...
    # ================================================================
    # 9. FLOW STAGE BINDINGS FOR oe-app-invalidation-flow
    # ================================================================
    ...
    wallet_ui_url = os.getenv("WALLET_UI_URL", "")
    participant_redirect_uris = []
    if wallet_ui_url:
        wallet_ui_url = wallet_ui_url.rstrip('/')
        participant_redirect_uris = [
    ...
    save_onboarding_config(app_slug, client_id, client_secret, base_url, participant_redirect_uris)
...
def main():
    ...
    if generated_env:
        redirect_uris = json.loads(generated_env.get("AUTHENTIK_REDIRECT_URIS", ""))
    else:
        redirect_uris = [
        ...
        print("No AUTHENTIK_REDIRECT_URIS found in .env.config, using defaults")
    
    logout_uri = generated_env.get("AUTHENTIK_LOGOUT_URI", "")
    ...
    print("    - oe-recovery-email (Email Stage with SMTP config from .env.config or defaults)")
    print("    - default-password-change-prompt (Prompt Stage)")
    print("    - default-password-change-write (User Write Stage)")
    print("  oe-enrollment-invitation Flow")
