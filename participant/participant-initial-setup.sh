...
BLUEPRINT_SCRIPT="authentik/participant_blueprint.py"
...
# 6. Run environment configuration from .env.config (generates .env.* files)
echo "==> Running $CONFIG_SCRIPT with $ENV_CONFIG..."
python "$CONFIG_SCRIPT"

# 7. Run the blueprint script (reads .env.config, generates blueprint YAML)
